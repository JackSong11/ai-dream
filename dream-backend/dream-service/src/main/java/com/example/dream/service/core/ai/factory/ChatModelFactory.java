...
import com.example.dream.service.core.ai.provider.ChatModelProvider;
 ...
 * ChatModel 工厂。
 * <p>
 * 启动时由 Spring 注入所有 {@link ChatModelProvider} 实现，
...
public class ChatModelFactory {
     ...
     * 协议类型 -> 提供者 的映射表。
     */
    private final Map<String, ChatModelProvider> providerMap;

    // 构造器注入，Spring容器在创建ChatModelFactory Bean的时候Spring 会分析：参数类型是 List<ChatModelProvider>。
    // 于是它会去 IOC 容器中找：所有实现了 ChatModelProvider 的 Bean，然后自动组成一个 List，这个 List 就作为参数传给构造器。
    // 注意，就算你有多个兼容openAi的baseurl，在配置文件里面写多个provider配置，也只能代表ProviderProperties有多个。
    // 在Spring的Bean容器里面，ChatModelProvider也会有一个，因为你的ChatModelProvider类代码只有一个，和配置文件的provider配置无关
    public ChatModelFactory(List<ChatModelProvider> providers) {
        this.providerMap = providers.stream()
                .collect(Collectors.toMap(ChatModelProvider::type, Function.identity()));
    }

    /**
     * 根据供应商与模型配置构建 ChatModel。
     *
    ...
    public ChatModel create(ProviderProperties provider, ModelProperties model) {
        ChatModelProvider impl = providerMap.get(provider.getType());
        if (impl == null) {
